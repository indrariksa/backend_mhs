...
func Homepage(c *fiber.Ctx) error {
	return c.SendString("Hello, Fiber! API Sudah Berjalan")
}
...
func GetMahasiswaByNPM(c *fiber.Ctx) error {
	// Mengambil NPM dari parameter URL dan mengkonversinya menjadi integer
	npmStr := c.Params("npm")
	...
	// Memanggil repository untuk mengambil mahasiswa berdasarkan NPM
	mhs, err := repository.GetMahasiswaByNPM(c.Context(), npm)
	if err != nil {
		// Menangani error jika mahasiswa tidak ditemukan atau error lainnya
		return c.Status(fiber.StatusNotFound).JSON(fiber.Map{
			"error": err.Error(),
		})
	}

	// Jika mahasiswa tidak ditemukan (mhs adalah nil), kembalikan data: null
	if mhs == nil {
		return c.Status(fiber.StatusNotFound).JSON(fiber.Map{
...
func InsertMahasiswa(c *fiber.Ctx) error {
	...
	// Bind JSON request body ke struct Mahasiswa
	...
	// // Generate ObjectID for Mahasiswa (if not set)
	// if mhs.ID.IsZero() {
...
func UpdateMahasiswa(c *fiber.Ctx) error {
	...
	// Convert npm from string to int
...
func DeleteMahasiswa(c *fiber.Ctx) error {
	...
	// Cek apakah npm kosong
	// if npm == "" {
	// 	return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
	// 		"error": "Parameter npm wajib diisi",
	// 	})
	// }
