...
func TestGetMahasiswaByNPM(t *testing.T) {
	npm := 2203101026 // NPM yang digunakan pada TestInsertMahasiswa
	mhs, err := repository.GetMahasiswaByNPM(ctx, npm)
	if mhs.NPM != npm {
		t.Errorf("TestGetMahasiswaByNPM failed: %v", err)
	} else {
...
func TestUpdateMahasiswa(t *testing.T) {
	...
	if err != nil {
		...
		fmt.Printf("Updated Mahasiswa with NPM: %d\n", updatedNPM)
	}
}
...
func TestDeleteMahasiswa(t *testing.T) {
	...
	if err != nil {
		...
		fmt.Printf("Deleted Mahasiswa with NPM: %d\n", deletedNPM)
	}
}
